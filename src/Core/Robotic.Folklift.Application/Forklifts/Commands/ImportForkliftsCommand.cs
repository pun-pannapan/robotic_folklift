...
namespace Robotic.Folklift.Application.Forklifts.Commands
{
    ...
    {
        private readonly IAppDbContext _db; private readonly ImportRowValidator _validator = new();
        public ImportForkliftsHandler(IAppDbContext db) { _db = db; }
        ...
        public async Task<ImportResultDto> Handle(ImportForkliftsCommand request, CancellationToken ct)
        {
            ...
            else if (ext == ".csv")
            {
                ...
                while (true)
                {
                    var line = reader.ReadLine(); if (line == null) break; if (string.IsNullOrWhiteSpace(line)) continue;
                    var parts = line.Split(','); if (parts.Length < 3) continue;
                    if (!DateTime.TryParse(parts[2], CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal, out var dt)) continue;
                    rows.Add(new ImportRow { Name = parts[0].Trim(), ModelNumber = parts[1].Trim(), ManufacturingDate = dt });
                }
            }
            else throw new InvalidOperationException("Unsupported file type. Use .csv or .json");

            var errors = new List<string>();
            var toInsert = new List<Forklift>();
            foreach (var row in rows)
            {
                var validate = await _validator.ValidateAsync(row, ct);
                if (!validate.IsValid) { errors.AddRange(validate.Errors.Select(e => $"{row.Name}: {e.ErrorMessage}")); continue; }


                bool exists = await _db.Forklifts.AnyAsync(f => f.Name == row.Name && f.ModelNumber == row.ModelNumber, ct);
                if (exists) continue;
                toInsert.Add(new Forklift { Name = row.Name, ModelNumber = row.ModelNumber, ManufacturingDate = row.ManufacturingDate });
            }
